...
const DEFAULTS = Object.freeze({
  ...
  theme: 'molan',
  /**
   * 选哪条常驻片段。常驻状态占 98% 的时间，所以这一条决定了"她平时什么样"。
   * 可选值即动作名（breathe / breatheSlow / blink …）；给的名字不存在时按
   * breathe → 任意常驻 → 第一条 依次回退。
   */
  idleAction: 'breathe',
...
function resolveConfig(rowConfig) {
  ...
  return Object.freeze({
    ...
    idleAction: String(merged.idleAction || DEFAULTS.idleAction),
